...
{
	set_pos(&sprite->verhit, 0, 0);
	set_pos(&sprite->horhit, 0, 0);
...
void	find_sprite_hit(t_game *game, t_ray *ray, t_sprite *sprite)
{
	...
	horz_hit_dist = 0;
	vert_hit_dist = 0;
	if (ray->sprite.horhit.x && ray->sprite.horhit.y)
		horz_hit_dist = hypot(game->player.x - ray->sprite.horhit.x,
				game->player.y - ray->sprite.horhit.y);
	if (ray->sprite.verhit.x && ray->sprite.verhit.y)
		vert_hit_dist = hypot(game->player.x - ray->sprite.verhit.x,
				game->player.y - ray->sprite.verhit.y);
	if (((horz_hit_dist && horz_hit_dist < vert_hit_dist) ||
				(horz_hit_dist && !vert_hit_dist)))
		copy_pos(&sprite->hit, &ray->sprite.horhit);
	else if (((vert_hit_dist && vert_hit_dist < horz_hit_dist) ||
				(vert_hit_dist && !horz_hit_dist)))
		copy_pos(&sprite->hit, &ray->sprite.verhit);
}
